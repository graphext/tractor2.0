<script lang="ts">
  ...
  export let icon = MagnifyingGlass;
...
>
  <div class="flex flex-col gap-1">
    ...
    {:else}
      <Select.Label class="text-sm text-base-content/60">{label}</Select.Label>
    {/if}

    <Select.Trigger
      class="w-[220px] flex items-center pl-3 pr-2 py-1 bg-neutral input-sm input rounded-full shadow-sm"
      aria-label={label}
    >
