<script lang="ts">
  ...
  import { Graph, QuestionMark } from "phosphor-svelte";
  import { ColumnMappings } from "$lib/types/columnMappings";
  import { GraphextType } from "$lib/types";

  let apifyClient = new ApifyClient(YOUTUBE_ACTOR_ID);
  const socialMedia = "youtube";

  const columnMapping = new ColumnMappings({
    date: GraphextType.DATE,
    title: GraphextType.TEXT,
    channelName: GraphextType.TEXT,
    numberOfSubscribers: GraphextType.NUMBER,
    type: GraphextType.CATEGORY,
    id: GraphextType.CATEGORY,
    url: GraphextType.URL,
    thumbnailUrl: GraphextType.URL,
    viewCount: GraphextType.NUMBER,
    likes: GraphextType.NUMBER,
    location: GraphextType.CATEGORY,
    channelUrl: GraphextType.URL,
    channelId: GraphextType.CATEGORY,
    duration: GraphextType.NUMBER,
    commentsCount: GraphextType.NUMBER,
    text: GraphextType.TEXT,
    subtitles: GraphextType.TEXT, // This will be an array of text
    commentsTurnedOff: GraphextType.BOOLEAN,
    fromYTUrl: GraphextType.URL,
    isMonetized: GraphextType.BOOLEAN,
    hashtags: "list[category]",
    formats: "list[category]",
  });
        ...
        <Tooltip.Root openDelay={0}>
          ...
          >
            <div class="prose prose-sm">
              <h3>Search Mode</h3>
              <p>Search in the same way as in the youtube search bar.</p>

              <p>
                To enter <b>url mode</b>, simply paste a youtube link to a
                video, channel, playlist, or results page.
              </p>

              <p>
                Date filtering is only enabled when in <b>url mode</b>.
              </p>

              <h3>Url Mode</h3>
              <p>
                Search for specific video, channel, playlist or results page
                URL.
              </p>

              <p>This mode enables date range filtering.</p>

              <p>You can input several links, just separate them by commas.</p>
            </div>
          </TooltipContent>
        </Tooltip.Root>
      </div>
          ...
          {:else}
            ...
            >
              <div class="prose prose-sm">
                ...
                <p>
                  You can input several links, just separate them by commas.
                </p>
              </div>
            </TooltipContent>
          {/if}
