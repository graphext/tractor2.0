...
Regarding Location and Geotags
...
Notation Regarding Time and Time Intervals

since:2021-12-31	On or after (inclusive) a specified date. 4 digit year, 2 digit month, 2 digit day separated by - a dash.	
...
within_time:2d within_time:3h within_time:5m within_time:30s	Search within the last number of days, hours, minutes, or seconds. Useful when user asks "in the past 3 days"
 			
Notation Regarding Tweet Types 
...
Notation Regarding App Specific stuff
...
Here are some guidelines that are extremely important when building queries:
...
By default "Top" results are shown, where "Top" means tweets with some engagements (replies, RTs, likes). "Latest" has most recent tweets. People search will match on descriptions, but not all operators work. 
...
For example: I want Tweets from @Nasa with all types of media except images

from:NASA filter:media -filter:images

Important and super cool: Combine complex queries together with booleans and parentheses to refine your results. Spaces are implicit logical AND, but OR must be explicitly included.
