<script lang="ts">
  ...
  import { ColumnMappings } from "$lib/types/columnMappings";

  let apifyClient = new ApifyClient(TIKTOK_ACTOR_ID);

  const columnMappings = new ColumnMappings({
    createTimeISO: "date",
    text: "text",
    webVideoUrl: "url",
    playCount: "number",
    shareCount: "number",
    diggCount: "number",
    "videoMeta.coverUrl": "url",
    top_5_hashtags: "list[category]",
    collectCount: "number",
    commentCount: "number",
    "authorMeta.name": "category",
    "authorMeta.nickName": "category",
    "authorMeta.signature": "text",
    "authorMeta.avatar": "url",
    "authorMeta.profileUrl": "url",
    "authorMeta.following": "number",
    "authorMeta.heart": "number",
    "authorMeta.video": "number",
    "authorMeta.digg": "number",
    ...
    "authorMeta.bioLink": "url",
    "authorMeta.privateAccount": "boolean",
    "authorMeta.ttSeller": "boolean",
    "authorMeta.fans": "number",
    mediaUrls: "list[url]",
    "videoMeta.height": "number",
    "videoMeta.width": "number",
    "videoMeta.duration": "number",
    "videoMeta.originalCoverUrl": "url",
    "videoMeta.definition": "text",
    "videoMeta.format": "text",
    mentions: "list[category]",
    effectStickers: "list[category]",
    isSlideshow: "boolean",
    isPinned: "boolean",
    isSponsored: "boolean",
    input: "text",
    "searchHashtag.views": "number",
    "searchHashtag.name": "text",
    "videoMeta.subtitleLinks": "list[url]",
    "authorMeta.roomId": "text",
  });
      ...
      onComplete: async ({
        ...
        const fileKeyWord =
          (hashtagArray.length > 0
            ? hashtagArray.map((k) => k.replaceAll(/[^\w\s]/gi, "")).join("_")
            : "") +
          (profileArray.length > 0
            ? profileArray.map((k) => k.replaceAll(/[^\w\s]/gi, "")).join("_")
            : "") +
          (keywordArray.length > 0
            ? keywordArray.map((k) => k.replaceAll(/[^\w\s]/gi, "")).join("_")
            : ""
          ).toLowerCase();
  ...
  async function handleTikTokSubmit() {
    ...
    hashtagArray = hashtagInput.split(",").map((e) => e.trim().toLowerCase());

    profileArray = profileInput.split(",").map((e) => e.trim().toLowerCase());

    keywordArray = keywordInput.split(",").map((e) => e.trim().toLowerCase());
          ...
          >
            <div class="prose prose-sm">
              ...
              <ul class="flex flex-col gap-2">
                <li>
                  <b># Hashtags</b>: collect data about videos containing this
                  hashtag: likes, users, followers and more.
                </li>
                <li>
                  <b>Usernames</b>: get info about specific profiles and their
                  videos
                </li>
                <li>
                  <b>Keywords</b>: you can search for particular keywords. We
                  will search for videos and profiles containing the keyword.
                </li>
              </ul>

              <p>
                You can search for multiple hashtags, usernames, or keywords
                simultaneously. Separate each item by commas. For example:
              </p>
      ...
      <div class="flex flex-col gap-5">
        <div class="flex justify-between items-baseline">
          ...
          {#if status == "ABORTED" || status == "READY"}
            <ResumeButton {status} bind:resuming {apifyClient} {runId} />
          {/if}
